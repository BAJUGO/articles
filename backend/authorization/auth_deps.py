...
def get_current_user_token(request: Request) -> AccessTokenData:
    try:
        token = get_token_from_cookies(request, token_type="access_token")
        if token is None:
            refresh_token = get_token_from_cookies(request, token_type="refresh_token")
...
def get_user_with_role(required_role: list[str]):
    def subfunction(
            user_token: AccessTokenData = Depends(get_current_user_token)
    ) -> AccessTokenData:
